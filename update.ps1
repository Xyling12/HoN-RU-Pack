...
function Get-DirectDropboxUrl($url) {
...
function Resolve-PackageRoot($extractDir) {
    $directMarker = Join-Path $extractDir "run_hon_full_translation.bat"
    if (Test-Path $directMarker) {
        return $extractDir
    }
    ...
    if ($dirs.Count -eq 1) {
        ...
        $nestedMarker = Join-Path $candidate "run_hon_full_translation.bat"
        if (Test-Path $nestedMarker) {
            return $candidate
        }
    }
