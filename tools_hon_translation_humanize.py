...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="OpenAI model for text polishing.",
    )
    p.add_argument(
        "--chunk-size",
        type=int,
...
def load_api_key() -> str:
    ...
    if sys.platform.startswith("win"):
        try:
            ...
            with winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"Environment") as key:
                reg_val, _ = winreg.QueryValueEx(key, "OPENAI_API_KEY")
    ...
    raise RuntimeError(
        "OPENAI_API_KEY not found. Set it in env or HKCU\\Environment first."
    )
...
def build_messages(batch: list[Candidate]) -> list[dict[str, str]]:
    sys_msg = (
